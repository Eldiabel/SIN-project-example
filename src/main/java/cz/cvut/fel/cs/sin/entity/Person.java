...
@Entity
@NamedQueries({
    @NamedQuery(name = "findPersonByLastName", query = "select p from Person p where p.lastName = :name"),
    @NamedQuery(name = "findPersonFromTeamByName", query = "select p from Team t, in(t.persons) p where t.name = :name")
